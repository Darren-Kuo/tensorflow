...
class Backtesting(object):
    ...
    def Backtest(self, df_test, tf_test):
        signal = self.Get_Signal(tf_test)
        
        _df_test = df_test.reset_index()
        _signal = pd.DataFrame(signal, columns = ['Signal']).shift(1)
    ...
    def Get_Signal(self, data):
        signal, _ = self.model.Predicting(data)
